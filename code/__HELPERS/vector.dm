...
/datum/plot_vector/proc/setup(var/turf/S, var/turf/T, var/xo = 0, var/yo = 0, var/click_x = (world.icon_size/2), var/click_y = (world.icon_size/2), var/angle_offset=0)
	source = S
	target = T
	...
	var/dx = ((target.x - source.x)*world.icon_size + (click_x - world.icon_size/2))/world.icon_size
	var/dy = ((target.y - source.y)*world.icon_size + (click_y - world.icon_size/2))/world.icon_size
...
/datum/plot_vector/proc/return_hypotenuse()
	return sqrt(((offset_x / world.icon_size) ** 2) + ((offset_y / world.icon_size) ** 2))
